...
namespace uTween
{
    public abstract class uTweener
    {
        ...
        event UnityAction onFinished = null;

        event UnityAction onUpdate = null;
        ...
        public virtual void StartTween() { enabled = true; }
        ...
        public void Sample(float factor, bool isFinished)
        {
            ...
            val = (method == EaseType.none) ? animationCurve.Evaluate(val) : EaseUtility.EasingFromType(0, 1, val, method);
        ...
        static public T Begin<T>() where T : uTweener
        {
            T comp = System.Activator.CreateInstance<T>();
            if (comp == null){
                Debug.LogError("Unable to create " + typeof(T));
            }
            comp.mStarted = false;
            comp.mFactor = 0f;
